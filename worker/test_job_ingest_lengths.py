...
def _apply_embedder_cap(text: str) -> Tuple[int, int]:
    try:
        cap = int(os.getenv("EMBEDDER_MAX_CHARS", "6000"))
    except Exception:
        cap = 6000
    cap = max(200, min(50000, cap))
    before = len(text)
    after = before if before <= cap else cap
    return before, after
...
def main() -> None:
    ...
    for idx, job in enumerate(latest, start=1):
        ...
        text_to_embed = (str(parsed.get("title", "") or "") + "\n" + desc).strip()
        before, after = _apply_embedder_cap(text_to_embed)
        print("text_to_embed_len_before_cap:", before)
        print("text_to_embed_len_after_cap:", after)
